...
class Ball:
    ...
    def check_collision(self, padd1, padd2):
        ...
        if self.rect.left <= -20:
            self.direction.x = 1
        elif self.rect.right >= self.display_w + 20:
            self.direction.y = -1 

        # collision with the left paddle
        if self.rect.colliderect(padd1):
            if self.rect.top <= padd1.bottom and self.direction.y < 0:
                self.direction.y = 1
            elif self.rect.bottom >= padd1.top and self.direction.y > 0:
                self.direction.y = -1
            if self.rect.left <= padd1.right and self.direction.x < 0:
                self.direction.x = 1

        # collision with the right paddle
        if self.rect.colliderect(padd2):
            if self.rect.top <= padd2.bottom and self.direction.y < 0:
                self.direction.y = 1
            elif self.rect.bottom >= padd2.top and self.direction.y > 0:
                self.direction.y = -1
            if self.rect.right >= padd2.left and self.direction.x > 0:
                self.direction.x = -1
